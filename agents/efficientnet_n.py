...
# from dataloader.dataloader import nih_gray_dataloader
...
class efficientnet_agent():
    def __init__(self, config):
        ...
        '''
        if config.gray :
            self.train_dataloader = nih_gray_dataloader(config, train="train")
            self.val_dataloader = nih_gray_dataloader(config, train="val")
            self.test_dataloader = nih_gray_dataloader(config, train="test")
        else :
        ...
        if config.pretrain :
            if config.gray :
                self.net=EfficientNet.from_pretrained(self.detail, in_channels=1, batch_norm_momentum=0.99)
            else:
                self.net=EfficientNet.from_pretrained(self.detail, batch_norm_momentum=0.99)
        else :
            if config.gray :
                self.net=EfficientNet.from_name(self.detail, in_channels=1)
            else:
                self.net=EfficientNet.from_name(self.detail)
        '''
        if config.pretrain == True :
            self.net=EfficientNet.from_pretrained(self.detail, batch_norm_momentum=0.99)
        else:
            self.net=EfficientNet.from_name(self.detail, batch_norm_momentum=0.99)
        ...
        self.output_image_img_path = config.output_path + '/image/ori/'
        self.output_image_cam_path = config.output_path + '/image/cam/'
        self.output_image_img_cam_path = config.output_path + '/image/img_cam/'
        ...
        make_dir(self.output_image_img_path)
        make_dir(self.output_image_cam_path)
        make_dir(self.output_image_img_cam_path)
    ...
    def train_one_epoch(self):
        ...
        for idx, (inputs, labels, _) in enumerate(train_tqdm_batch):
            ...
            corr += (torch.round(input=outputs) == labels ).float().sum()
            acc = corr / len(self.train_dataloader.train_loader)
            self.train_acc.update(acc)

            train_tqdm_batch.set_description("epochs={:5d}, loss={:0.4f}, accuracy={:0.4f}".format(self.current_epoch, loss, self.train_acc.val))
        ...
        with open(self.log_path_train, 'a', newline='') as train_writer_csv:
            ...
            train_writer.writerow([self.current_epoch, str(self.epoch_train_loss.val), str(scheduler_lr) ,str(self.train_acc.val)])
    ...
    def val(self):
        ...
        for val_idx, (inputs, labels, _) in enumerate(val_tqdm_batch):
            ...
            val_tqdm_batch.set_description("epochs={:5d}, loss={:0.4f}, acc={:0.4f}".format(self.current_epoch, self.epoch_val_loss.val, self.val_acc.val))
        ...
        with open(self.log_path_val, 'a', newline='') as val_writer_csv:
            ...
            val_writer.writerow([self.current_epoch, str(self.epoch_val_loss.val), str(self.val_acc.val)])
    ...
    def test(self):
        ...
        for test_idx, (inputs, labels, _) in enumerate(test_tqdm_batch):
            ...
            for j in range(0, batch_size):
                ...
                for k in range(len(self.Pred_label)):
                    thisrow["prob_" + self.Pred_label[k]] = probs[j, k]
                    truerow[self.Pred_label[k]] = true_labels[j, k]
    ...
    def visual(self):
        ...
        for file_name in file_names:
            ...
            cam, preds = grad_cam.generate_cam(img_tensor)

            print("save! :"+ self.input_path + file_name, gen_count)
            
            heatmap, heatmap_on_img = apply_colormap(img_pil_resize, cam)

            img_pil.save(self.output_image_img_path + str(np.argmax(preds)) + "_" + file_name)
            heatmap.save(self.output_image_cam_path + str(np.argmax(preds)) + "_" + file_name)
            heatmap_on_img.save(self.output_image_img_cam_path + str(np.argmax(preds)) + "_" + file_name)
            ...
            with open(self.output_pred_path + 'preds.csv', 'a', newline='') as pred_writer_csv:
                ...
                pred_writer.writerow([file_name, \
                        str(preds)])
